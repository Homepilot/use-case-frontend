...
let UnitService = class UnitService {
    constructor(prismaService, landlordService) {
    ...
    async updateUnit(id, data) {
        const unit_to_update = await this.prismaService.unit.findUnique({
            where: { id },
        });
        if (!unit_to_update)
    ...
    async deleteUnit(id) {
        const unit_to_delete = await this.prismaService.unit.findUnique({
            where: { id },
        });
        if (!unit_to_delete)
            throw new common_1.HttpException('Unit not found', common_1.HttpStatus.NOT_FOUND);
